...
const UpdateModal: React.FC<UpdateModalProps> = ({
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/equipe_user/${team.id}`
        );
    ...
    const fetchUsers = async (teamMembers: Member[]) => {
      try {
        ...
        const userWithOutTeam = response.data.filter(
          (user: User) => !teamMembers.some((member) => member.id === user.id) // Use teamMembers atualizado
        );
  ...
  const handleMemberChange = (
    ...
    setMembers((prevMembers) => {
      ...
      if (field === "role" && value !== previousRole) {
        ...
        const remainingLeaders = updatedMembers.filter(
          (m) => m.role === "Líder"
        ).length;
  ...
  const handleDeleteMember = async (memberId) => {
    const confirmDelete = window.confirm(
      `Você realmente deseja excluir este membro da equipe?`
    );
    ...
    if (confirmDelete) {
      try {
        ...
        if (removerUser) {
          setUsers((prevUsers) => [
            ...prevUsers,
            { id: memberId, nome: removerUser.name },
          ]);
        }
  ...
  const updateTeam = async () => {
    try {
      const response = await axios.put(
        `http://localhost:3000/equipe/${team.id}`,
        {
          nome: teamName,
      ...
      if (member.name && member.role) {
        ...
        if (user) {
          ...
          if (!isAlreadyMember && !isDuplicate) {
            ...
            console.warn(
              `Usuário ${member.name} já está associado ou duplicado. Ignorando...`
            );
          }
        } else {
        ...
        console.warn(
          `Membro ${JSON.stringify(
            member
          )} não possui nome ou função. Ignorando...`
        );
      }
    });
  ...
  const handleAssociateUser = async (userId: number, isLider: boolean) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/equipe_user/associar",
        {
          user_id: userId,
          equipe_id: team.id,
          is_lider: isLider,
  ...
  const handleChangeLeader = async (memberId: number, isLeader: boolean) => {
    try {
      ...
      console.log(
        `Membro com ID ${memberId} agora é ${
          isLeader ? "líder" : "liderado"
        } da equipe.`
      );
    } catch (error) {
      ...
      if (
        error.response &&
        error.response.data.message === "A equipe deve ter pelo menos um líder."
      ) {
        alert(
          "Não é possível remover o último líder da equipe. Adicione outro líder antes de remover este."
        );
      }
    }
  };
