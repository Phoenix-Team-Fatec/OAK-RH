...
export default function UserFormsResponseView() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const usersArrayResponse = await axios.get(`http://localhost:3000/formulario_equipe/get/arrayUserAnswered/${formsId}/${id}`);
                ...
                if (usersArrayResponse.data.length > 0) {
                    ...
                    const answersDataAPI = await axios.get(`http://localhost:3000/respostas/forms/user/${formulario_id}/${firstUserId}`)
    ...
    async function nextUser() {
        ...
        await axios.get(`http://localhost:3000/user/getData/${nextUserId}`)
        ...
        const answersDataAPI = await axios.get(`http://localhost:3000/respostas/forms/user/${formulario_id}/${nextUserId}`)
    ...
    async function previousUser() {
        ...
        await axios.get(`http://localhost:3000/user/getData/${prevUserId}`)
            .then(userDataResponse => {
                setUser(userDataResponse.data.nome);
            })
            .catch(error => {
        ...
        const answersDataAPI = await axios.get(`http://localhost:3000/respostas/forms/user/${formulario_id}/${prevUserId}`)
            ...
            {isLoading ? (
                ...
                <>
                    <div className="container-forms-ver-respostas-user">
                        ...
                        {
                            (!formsData || formsData.length === 0 || !answerData || answerData.length === 0)
                                ? <p>Carregando...</p>
                        ...
                        <div className="responseNavigationButtonContainer">
                            ...
                            <button onClick={() => handleNavigation('next')}>Próximo</button>                        </div>
                    </div>
                </>
            )}
        </div>
